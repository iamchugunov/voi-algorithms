function [KFilter] = PDKalmanFilter3D_T(track, config, X0, s_ksi)
    poits = track.poits;
    ...
    for i = 2:length(poits)
        ...
        [X(:,i), Dx, discr] = Kalman_step_3Dpd_T(poits(i).ToA*config.c_ns + poits(i).Frame * config.c, X(:,i-1), Dx, dt, s_n, D_ksi, config); 
        Dx_hist(:,i) = diag(Dx);
    end
