...
close all
  ...
  X0 = [track.poits(1).true_crd(1,1); 
      ...
      track.poits(1).true_vel(3,1);
      track.poits(1).true_ToT];
  KFilter_res_pd = PDKalmanFilter3D_T(track, config, X0, sigma_ksi, measurements_params.period_sec);
...
%   [track.poits, res] = thinning_measurements(track.poits, params, config);
  ...
  sigma_ksi = 10;
  KFilter_res_rd1 = RDKalmanFilter3D(track, config, X0, sigma_ksi);
  sigma_ksi = 0.0;
  KFilter_res_rd2 = RDKalmanFilter3D(track, config, X0, sigma_ksi);
%   for i = 1:length(track.poits)
%     track.poits(i) = crd_calc(track.poits(i),config);
%   end
