...
t0_real = real_track.t(num) - 2.2; % SDVIG PRI KOTOROM MAXIMALNAYA KORRELATCIYA
...
[KFilter] = RDKalmanFilter3D_hgeo_inpoits(track, config, res.X, Dx0, sigma_ksi,50);
% [KFilter] = RDKalmanFilter3D(track, config, X0, Dx0, sigma_ksi);

show_posts2D
% daspect auto
show_primary_points2D(cur_poits)
plot(KFilter.crd(1,:)/1000,KFilter.crd(2,:)/1000,'.-')
plot(real_track_new.crd(1,:)/1000,real_track_new.crd(2,:)/1000,'.-k')
plot(res.crd(1,:)/1000,res.crd(2,:)/1000,'.-b')

err_calc_real_log(real_track_new, KFilter)
