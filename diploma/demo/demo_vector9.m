...
Filter = RDKalmanFilter3D_Accumulation(track, config, s_ksi, X0, Dx0, paramsT);
...
%%
function [KFilter] = RDKalmanFilter3D_Accumulation(track, config, s_ksi, ...
    X0, Dx0, params_disk)
...
%             [dt length(current_poits)]
            [X, Dx, d] = Kalman_step_3Drd_Accumulation(current_poits, X_prev, Dx, s_n, D_ksi, config, dt);
...
%%
function [X, Dx, ud] = Kalman_step_3Drd_Accumulation(poits, X_prev, Dx, sigma_n, D_ksi, config, T)
