...
% Dx0 = [956.519629293195	47.0020408432237	-1003.87675022495	-49.3274783021108	540.363373210078	26.5329799680037;
% 47.0020408432233	4.12149586511767	-49.3275013727275	-4.32483450669779	26.5323320939753	2.31907806070115;
% -1003.87675022495	-49.3275013727280	1056.60663249883	51.9195668884296	-570.162070371168	-27.9980762442171;
% -49.3274783021104	-4.32483450669779	51.9195668884291	4.55241029594927	-27.9973811813562	-2.44772576500676;
% 540.363373210079	26.5323320939755	-570.162070371168	-27.9973811813564	409.405434187650	20.1886302681571;
% 26.5329799680036	2.31907806070115	-27.9980762442168	-2.44772576500676	20.1886302681571	1.79181212759872];
s_ksi = 1;
...
function [ud, Dx, discr] = make_ud_Dx(poits, X_ext, D_x_ext, sigma_n, config)
    ...
    for i = 1:length(poits)
        ...
        [w, ud_, d] = make_w_ud(y, X_, config.posts, sigma_n, dt);
        W = W + w;
        ud = ud + ud_;
        discr = [discr d];
    end
...
function [w, ud, d] = make_w_ud(y, X, posts, sigma_n, dt)
    ...
    dS = [];
    for i = 1:length(nums)
        S(i,1) = norm(X([1 3 5],:) - posts(:,nums(i)));
        dsdx = (X(1,1) - posts(1,nums(i)))/S(i,1);
        dsdy = (X(3,1) - posts(2,nums(i)))/S(i,1);
        dsdz = (X(5,1) - posts(3,nums(i)))/S(i,1);
